...
public class SignInTest extends Setup {
    ...
    @Test(priority = 1, groups = "enabledCheck")
    void testPasswordFieldEnabled() {
        assertTrue(sign.passwordFieldEnabled());
    }

    @Test(priority = 2, groups = "enabledCheck")
    void testCreateAccountButtonEnabled() {
        assertTrue(sign.createAccountButtonEnabled());
    }

    @Test(priority = 3)
        // Succeeds in sending the information and test passes, but the page does not load further
    ...
    @Test(priority = 5, groups = "gotoLinkCheck")
